...
export default function ConvertedLeadsPage() {
  ...
  async function handleRestoreToInterested(leadId: string) {
    try {
      const supabaseAny = supabase as any;
      await supabaseAny
        .from('leads')
  ...
  return (
    <div className="p-6" style={{ minHeight: '100vh', background: '#1a1a1a', color: '#e5e5e5' }}>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#f3f4f6' }}>Converted Clients</h1>
          <p className="text-sm" style={{ color: '#9ca3af' }}>Successfully converted leads from your campaigns</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 rounded-lg shadow" style={{ background: '#2a2a2a' }}>
          ...
          <p style={{ color: '#9ca3af' }}>
            {search ? "No leads found matching your search" : "No converted leads yet."}
          </p>
          <button
            onClick={() => router.push('/app/dashboard/leads/imported')}
                ...
                <tr key={lead.id} style={{ background: '#2a2a2a' }}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                    <div className="flex gap-2 justify-end">
                      <button
                        onClick={() => handleRestoreToInterested(lead.id)}
                        style={{
                          background: '#4facfe',
                          color: 'white',
                          padding: '0.5rem 1rem',
