...
COMMENT ON COLUMN inventory_items.purchase_cost IS 'Original purchase price of the item';
COMMENT ON COLUMN inventory_items.purchase_date IS 'Date the item was purchased';
COMMENT ON COLUMN inventory_items.useful_life_years IS 'Expected useful life in years';
COMMENT ON COLUMN inventory_items.estimated_jobs_per_year IS 'Estimated number of jobs this item is used on per year';
COMMENT ON COLUMN inventory_items.residual_value IS 'Expected resale/scrap value at end of useful life';
COMMENT ON COLUMN inventory_items.amortization_per_job IS 'Auto-calculated: cost recovery per job based on depreciation';
