...
-- Adds role-based training assignment and tracking system
...
CREATE TABLE IF NOT EXISTS public.training_progress (
  ...
  last_position INTEGER DEFAULT 0, -- last position in video
...
CREATE TABLE IF NOT EXISTS public.training_tests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  module_id UUID REFERENCES public.training_modules(id) ON DELETE CASCADE,
  category TEXT NOT NULL, -- can group multiple modules into one test
  title TEXT NOT NULL,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS public.test_submissions (
  ...
  answers JSONB NOT NULL, -- stores question_id -> answer mapping
