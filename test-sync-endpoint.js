...
async function testSyncEndpoint() {
  ...
  try {
    ...
    const passed = response.status === 200 && data.synced === 0;
    ...
    console.log(`  ${passed ? '✅' : '❌'} Status: ${response.status}, Synced: ${data.synced}`);
  } catch (error) {
  ...
  try {
    ...
    const data = await response.json();
    const passed = response.status === 200 && data.failed > 0;
    ...
    console.log(`  ${passed ? '✅' : '❌'} Status: ${response.status}, Failed: ${data.failed}`);
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    results.push({
      name: 'Invalid table name',
